...
public interface PathRepository {
    ...
    @Insert("insert into inspection_path (number,area_belong,pipe_belong,startpoint,endpoint,create_date,description,name,drawpoint) " +
            "values (#{number},#{area_belong},#{pipe_belong},#{startpoint},#{endpoint},#{create_date},#{description},#{name},#{drawpoint})")
    ...
    @Update("update inspection_path set number = #{number},area_belong = #{area_belong},pipe_belong = #{pipe_belong},startpoint = #{startpoint}," +
            "endpoint = #{endpoint},create_date = #{create_date},description = #{description}, name = #{name}, drawpoint=#{drawpoint} where id = #{id}")
